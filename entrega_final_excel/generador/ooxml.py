...
def _attr(texto):
    """Escapa para meter texto dentro de un atributo XML (comillas incluidas)."""
...
class Pivot:
    """
    Tabla dinamica de una dimension en filas y una o varias medidas.

    Con una sola medida el valor va directo bajo el encabezado. Con dos o mas,
    las medidas se despliegan en columnas usando el campo virtual "Valores"
    (field x="-2"), que es como Excel modela el area de datos.
    """

    def __init__(self, name, sheet, anchor_row, anchor_col, row_field,
                 data_field=None, caption=None, data_fields=None,
                 row_caption=None):
        self.name = name
        self.sheet = sheet
        ...
        # data_fields: [(campo, titulo), ...]
        if data_fields is None:
            data_fields = [(data_field, caption)]
        self.data_fields = list(data_fields)

    @property
    def data_field(self):
        return self.data_fields[0][0]

    @property
    def caption(self):
        return self.data_fields[0][1]
    ...
    @property
    def width(self):
        """Columnas que ocupa: la de etiquetas mas una por medida."""
        return 1 + len(self.data_fields)
...
def inject_pivots(path, source_sheet, columns, rows, pivots, slicer_fields=()):
    pkg = _Pkg(path)
    measures = {campo for p in pivots for campo, _ in p.data_fields}
    ...
    for pi, p in enumerate(pivots, start=1):
        rf = fidx[p.row_field]
        dfs = [fidx[campo] for campo, _ in p.data_fields]
        ...
        # con dos o mas medidas, el area de datos se abre en columnas usando
        # el campo virtual "Valores" (x="-2"); con una sola no hace falta
        if len(dfs) > 1:
            col_xml = ('<colFields count="1"><field x="-2"/></colFields>'
                       f'<colItems count="{len(dfs)}">'
                       + "".join("<i><x/></i>" if k == 0
                                 else f'<i i="{k}"><x v="{k}"/></i>'
                                 for k in range(len(dfs)))
                       + "</colItems>")
        else:
            col_xml = '<colItems count="1"><i/></colItems>'
        loc = (f"{col_letter(p.anchor_col)}{p.anchor_row}:"
               f"{col_letter(p.anchor_col + len(dfs))}{p.anchor_row + n + 1}")
        data_xml = (f'<dataFields count="{len(dfs)}">'
                    + "".join(f'<dataField name="{escape(cap)}" fld="{d}"'
                              f' baseField="0" baseItem="0"/>'
                              for d, (_, cap) in zip(dfs, p.data_fields))
                    + "</dataFields>")
        pkg.put(f"xl/pivotTables/pivotTable{pi}.xml", DECL
                ...
                + (f' rowHeaderCaption="{escape(p.row_caption)}"'
                   if p.row_caption else "")
                + f''
...
def titulos_dinamicos(path, titulos, color="1F3864", fuente="Arial"):
    ...
    for name in sorted(n for n in pkg.parts
        ...
        # el texto viene de una formula: puede traer & o < y hay que escaparlo
        texto = escape(valores[hoja].get(celda, ""))
        titulo = (
...
def traducir_cadenas(path, hoja, mapa):
    """
    Reemplaza textos cacheados de una hoja, despues del recalculo.

    LibreOffice materializa las tablas dinamicas con sus propios rotulos en
    ingles ("Data", "Total Result"). Excel los reescribe en castellano al
    refrescar, pero el archivo se entrega con los valores cacheados a la
    vista, asi que hay que dejarlos ya traducidos.

    Las celdas afectadas pasan de cadena compartida a cadena en linea, que es
    la forma mas simple de cambiar un texto sin reindexar sharedStrings.
    """
    pkg = _Pkg(path)
    if "xl/sharedStrings.xml" not in pkg.parts:
        return 0
    xml_ss = pkg.txt("xl/sharedStrings.xml")
    cadenas = ["".join(re.findall(r"<t[^>]*>(.*?)</t>", m.group(1), re.S))
               for m in re.finditer(r"<si>(.*?)</si>", xml_ss, re.S)]

    part = pkg.sheet_part(hoja)
    puestas = 0

    def _repl(m):
        nonlocal puestas
        celda, attrs, idx = m.group(1), m.group(2), int(m.group(3))
        if idx >= len(cadenas) or cadenas[idx] not in mapa:
            return m.group(0)
        puestas += 1
        attrs = re.sub(r'\st="s"', ' t="inlineStr"', attrs)
        return (f'<c r="{celda}"{attrs}><is><t>'
                f"{escape(mapa[cadenas[idx]])}</t></is></c>")

    xml = re.sub(r'<c r="([A-Z]+\d+)"([^>]*\st="s"[^>]*)><v>(\d+)</v></c>',
                 _repl, pkg.txt(part))
    pkg.put(part, xml)
    pkg.save()
    return puestas
...
def borrar_leyenda(path, specs):
    """
    Saca entradas concretas de la leyenda de un grafico.

    La cascada se dibuja apilando una serie base invisible; esa serie no
    significa nada para quien lee el tablero y no tiene que aparecer en la
    leyenda. LibreOffice no sabe borrar entradas sueltas, asi que se hace aca.

    specs: [(marcador, [indices de serie a borrar])]
    """
    pkg = _Pkg(path)
    borradas = 0
    for name in sorted(n for n in pkg.parts
                       if re.match(r"xl/charts/chart\d+\.xml$", n)):
        xml = pkg.txt(name)
        spec = next((s for s in specs if s[0] in xml), None)
        if spec is None or "<c:legend>" not in xml:
            continue
        entradas = "".join(
            f'<c:legendEntry><c:idx val="{k}"/><c:delete val="1"/>'
            f"</c:legendEntry>" for k in spec[1])
        # el esquema exige legendEntry justo despues de legendPos
        nuevo, n = re.subn(r"(<c:legendPos[^>]*/>)", r"\1" + entradas, xml,
                           count=1)
        if n:
            pkg.put(name, nuevo)
            borradas += len(spec[1])
...
def _dlbls_xml(num_fmt, pos, ocultar=(), size=700, color="2F3437",
               fuente="Arial"):
    """
    Bloque <c:dLbls>. El orden de los hijos lo fija el esquema, no el gusto:
    primero los <c:dLbl> de cada punto, despues el formato general.

    `ocultar` son los indices de punto que no llevan etiqueta. Sirve para
    etiquetar solo el ultimo valor de una serie larga, que es lo legible.
    """
    return (
        "<c:dLbls>"
        + "".join(f'<c:dLbl><c:idx val="{k}"/><c:delete val="1"/></c:dLbl>'
                  for k in ocultar)
        # ojo: los formatos de numero llevan comillas dobles ("$"#,##0," k")
        # y van dentro de un atributo, asi que hay que escaparlas tambien
        + (f'<c:numFmt formatCode="{_attr(num_fmt)}" sourceLinked="0"/>'
           if num_fmt else "")
        + '<c:spPr><a:noFill/><a:ln><a:noFill/></a:ln></c:spPr>'
        f'<c:txPr><a:bodyPr/><a:lstStyle/><a:p><a:pPr>'
        f'<a:defRPr sz="{size}" b="0">'
        f'<a:solidFill><a:srgbClr val="{color}"/></a:solidFill>'
        f'<a:latin typeface="{fuente}"/></a:defRPr></a:pPr>'
        f'<a:endParaRPr lang="es-AR"/></a:p></c:txPr>'
        + (f'<c:dLblPos val="{pos}"/>' if pos else "")
...
def etiquetas_de_datos(path, specs):
    """
    Pone etiquetas de datos en las series indicadas, despues del recalculo.

    Hay que hacerlo aca porque LibreOffice reescribe el bloque <c:dLbls> a su
    manera al guardar -deja las etiquetas apagadas y sin formato-, y las
    etiquetas son un requisito del entregable ("etiquetas de datos claras"),
    no un adorno. Si la serie ya trae un bloque, se reemplaza; si no, se
    inserta antes de <c:cat>/<c:xVal>, que es donde lo pide el esquema.

    specs: [(marcador, [indices de serie], numFmt, pos, [puntos a ocultar])]
           indices de serie = None -> todas las series del grafico
    """
    pkg = _Pkg(path)
    puestas = 0
    for name in sorted(n for n in pkg.parts
        ...
        if spec is None:
            continue
        _, series_idx, num_fmt, pos = spec[:4]
        ocultar = spec[4] if len(spec) > 4 else ()
        bloque = _dlbls_xml(num_fmt, pos, ocultar)

        partes, i, k = [], 0, 0
        for m in re.finditer(r"<c:ser>.*?</c:ser>", xml, re.S):
            ser = m.group(0)
            if series_idx is None or k in series_idx:
                if "<c:dLbls>" in ser:
                    ser = re.sub(r"<c:dLbls>.*?</c:dLbls>", bloque, ser,
                                 count=1, flags=re.S)
                    puestas += 1
                else:
                    anchor = ("<c:cat>" if "<c:cat>" in ser else
                              "<c:xVal>" if "<c:xVal>" in ser else None)
                    if anchor:
                        ser = ser.replace(anchor, bloque + anchor, 1)
                        puestas += 1
            partes.append(xml[i:m.start()])
            partes.append(ser)
            i = m.end()
            k += 1
        partes.append(xml[i:])
        pkg.put(name, "".join(partes))
    pkg.save()
    return puestas
...
def finalize(path, slicers, slicer_sheet, gridless_sheets=(),
    ...
    # --- y tambien los encabezados de fila/columna del tablero ------------
    for sname in headerless_sheets:
        pn = pkg.sheet_part(sname)
        x = pkg.txt(pn)
        if "showRowColHeaders" in x:
            x = re.sub(r'showRowColHeaders="[^"]*"', 'showRowColHeaders="0"',
                       x, count=1)
        else:
            x = x.replace("<sheetView ", '<sheetView showRowColHeaders="0" ', 1)
        pkg.put(pn, x)
