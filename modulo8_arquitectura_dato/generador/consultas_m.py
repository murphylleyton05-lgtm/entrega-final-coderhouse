"""
El codigo M del flujo ETL.

Vive en un solo lugar (`SECCION`) y desde aca se usa tres veces:

    1. se empaqueta dentro del .xlsx como DataMashup, para que las consultas
       aparezcan de verdad en el panel Consultas y conexiones,
    2. se escribe a `powerquery/Section1.m`, para poder leerlo o pegarlo en el
       Editor avanzado sin abrir el binario,
    3. se vuelca a la hoja `Codigo_M` del libro, para que se pueda auditar sin
       tener Power Query a mano.

Sobre el orden de los pasos: en `Hechos_Ventas` primero se filtra y recien
despues se tipa. Es al reves de lo que propone la interfaz por defecto, y es a
proposito: tipar 3.090 filas para despues descartar 90 es trabajo tirado, y
ademas las filas sucias son justamente las que romperian el tipado.
"""

# Nombre de cada consulta y para que esta. El orden es el de carga del modelo.
CONSULTAS = [
    ("pRutaFuentes", "Parametro: carpeta de los archivos de origen"),
    ("Hechos_Ventas", "Tabla de hechos: transacciones de venta"),
    ("Dim_Clientes", "Dimension: clientes (PK ID_Cliente)"),
    ("Dim_Productos", "Dimension: productos (PK ID_Producto)"),
    ("Dim_Calendario", "Dimension: calendario generado en M (PK Fecha)"),
]

SECCION = r'''section Section1;
